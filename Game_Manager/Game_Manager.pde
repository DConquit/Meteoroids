/******************************************************************************
 ...
 Game:         Asteroids - COSC101 Assignment 3
 ...
 References:   Hyperspace Bold font was obtained from: 
               ...
               Explosion sound effect was sourced from:
               https://www.freesoundeffects.com/free-track/explosion-3-466448/
               
               Bullet sound effect was sourced from:
               http://soundbible.com/1875-Bullet-Whizzing-By.html
               
               Background music was sourced from:
               http://www.orangefreesounds.com/life-space-synth-loop/
 
******************************************************************************/
import ddf.minim.*;
Minim minim = new Minim(this);
AudioPlayer themeMusic;           //Theme music of the game.
AudioSample bulletSound;          //Bullet firing sound effect.
...
void setup() {
  ...
  //Instantiation of sound objects.
  explosionSound = minim.loadSample("Explosion.wav");  
  bulletSound = minim.loadSample("Bullet.wav");
  themeMusic = minim.loadFile("SpaceMusic.wav");

  if (musicPlaying == false) {
    
    themeMusic.setGain(-20);
    themeMusic.loop();
    musicPlaying = true;
  }
...
void startDraw(int count) {
  ...
  asteroidImg = loadImage("asteroidImg.png");         //Load asteroid image.    
...
void keyPressed() { 
  ...
  if (player.getLives() > 0) {
    ...
    if (keyCode == ' ' && !gameOver && !bulletFire && 
      player.getSpreadUpgrade() == false && startGame && 
      player.getExploding() == false) {

      bulletSound.setGain(150);               //Volume of sound effect.
      bulletSound.trigger();                  //Play bullet sound effect.
    ...
    // Player 1 shooting with spread gun
      ...
      //bulletSound.amp(0.8);               //Volume of sound effect.
      bulletSound.trigger();                 //Play bullet sound effect.
